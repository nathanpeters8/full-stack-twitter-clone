    ...
    return (
      <div className='col-12 col-md-9 mt-4 mt-md-5'>
        ...
        <div className='content col-12 d-flex flex-column align-items-center mt-5'>
          <button className={'btn btn-warning align-self-start ' + (showSearchResults ? '' : 'd-none' )} onClick={() => {window.location.href = '/home';}}>Back</button>
              ...
              return tweets.map((tweet) => (
                <div className='tweet card border mb-5' key={tweet.id}>
                  <div className='card-body d-flex flex-column'>
                    <button
                      className={'btn btn-sm border align-self-end' + (currentUser !== tweet.username ? ' d-none' : '')}
                      onClick={() => this.handleDeleteTweet(tweet.id)}
                    >
                      Delete
                    </button>
                    <Link to={`/home/${tweet.username}`} className='h6 btn fw-light p-0 align-self-start'>
                      @{tweet.username}
                    </Link>
                    <hr />
                    <h5 className=''>{(showSearchResults && searchTerm !== '' ? this.highlightSearchTerm(tweet.message, searchTerm) : tweet.message)}</h5>
